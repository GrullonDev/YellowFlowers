...
class FlorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final talloPaint = Paint()
      ..shader = LinearGradient(
        ...
        begin: Alignment.bottomCenter,
          ...
          Rect.fromLTWH(center.dx - 2, center.dy, 4, size.height - center.dy))
      ..strokeWidth = 3.5
    ...
    for (var i = 0; i < 6; i++) {
      ...
      canvas.drawOval(
          Rect.fromCenter(center: const Offset(3, -28), width: 22, height: 52),
          petalSombraPaint);
      canvas.restore();
    }
    ...
    for (var i = 0; i < 6; i++) {
      final rect =
          Rect.fromCenter(center: const Offset(0, -30), width: 24, height: 56);
    ...
    final centroPaint = Paint()
      ..shader = const RadialGradient(
        colors: [
          Color(0xFF422100), // Centro profundo
          Color(0xFFE65100), // Medio
          Color(0xFFFB8C00), // Brillo exterior
        ],
    ...
    for (int p = 0; p < 8; p++) {
      ...
      canvas.drawCircle(
        Offset(
            center.dx + math.cos(angle) * 10, center.dy + math.sin(angle) * 10),
        1.5,
        polenPaint,
      );
    }
